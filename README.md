...
- **Backend (Render)**:
   ...
   - Build: Render will run `npm install` in the `server` folder automatically.
   - Start Command: `npm start` (the `server/package.json` includes this script).
   - Environment variables to set in Render:
      - `MONGODB_URI` — your MongoDB connection string
      - `JWT_SECRET` — secret for signing tokens (if used)
      - `ADMIN_PASSWORD` — admin password (optional)
      - `PORT` — Render sets this automatically; do not hard-code a different value
   - After deployment, note the service URL (for example `https://your-service.onrender.com`).

- **Frontend (Vercel)**:
   - Import the same repository into Vercel and set the **Root Directory** to `/client`.
   - Framework Preset: Vite (or select default React + Vite).
   - Build Command: `npm run build`
   - Output Directory: `dist`
   - Environment variable to set in Vercel:
      - `VITE_API_BASE_URL` — set this to your Render backend URL (for example `https://your-service.onrender.com`).
   - Deploy; the site will call the Render backend using `VITE_API_BASE_URL`.

Notes:
   - The frontend reads `VITE_API_BASE_URL` at build time and sets `axios.defaults.baseURL` accordingly.
   - The backend uses CORS with a permissive configuration by default, so the frontend should be able to call it once you set the correct `VITE_API_BASE_URL`.
   - Provide the `MONGODB_URI` and any secrets in the Render dashboard before first deploy.
